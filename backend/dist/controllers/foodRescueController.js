...
exports.getRescueClaims = exports.claimFoodListing = exports.createFoodListing = exports.getFoodListings = void 0;
const supabase_1 = require("../config/supabase");
// Helper for urgency scoring (hours till expiry)
const calculateUrgency = (expiryDate) => {
    const hoursLeft = (expiryDate.getTime() - Date.now()) / (1000 * 60 * 60);
    if (hoursLeft <= 0)
        return 0; // expired
    if (hoursLeft <= 24)
        return 10; // High urgency
    if (hoursLeft <= 72)
        return 5;
    return 2;
};
...
const createFoodListing = async (req, res) => {
    ...
    const { data, error } = await supabase_1.supabase.from('food_listings').insert([{
...
const claimFoodListing = async (req, res) => {
    ...
    const claimReq = await supabase_1.supabase.from('rescue_claims').insert([{
            claimer_id, food_listing_id: id, message, status: 'approved'
        }]).select().single();
    if (claimReq.error)
        return res.status(400).json({ error: claimReq.error.message });
    const { data, error } = await supabase_1.supabase.from('food_listings').update({ status: 'claimed' }).eq('id', id).select().single();
    if (error)
        return res.status(400).json({ error: error.message });
    res.json({ claim: claimReq.data, food_listing: data });
};
exports.claimFoodListing = claimFoodListing;
const getRescueClaims = async (req, res) => {
    const { userId } = req.params;
    const { data, error } = await supabase_1.supabase.from('rescue_claims').select('*, food_listings(*)').eq('claimer_id', userId);
    if (error)
        return res.status(400).json({ error: error.message });
    res.json(data);
};
exports.getRescueClaims = getRescueClaims;
